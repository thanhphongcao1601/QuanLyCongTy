    ...
    <div class="wrapper">
        ...
        <div class="row">
            <div class="col-12 col-sm-4 col-md-3 col-lg-2" id="left-bar">
                <div class="hadd-task" data-toggle="modal" data-target="#addtask-modal">
                    <i class="fas fa-pencil-alt"></i><b class="ml-2">Tạo task</b>
                </div>
                <hr />
                ...
                <div class="hsidebar-filter" id="cancel">Đã hủy</div>
            ...
            <div class="col-12 col-sm-8 col-md-9 col-lg-10">
                ...
                <div id="list" class="hscroll">
                    <!-- load danh sach task -->
                    <?php
                    $sql = "SELECT * FROM task WHERE idtp =" . $truongphong . " ORDER BY idtask DESC";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            if ($row['status'] == "completed") {
                                echo "<div class='hnotice hnotice-success' onclick='" . "location.href=\"cttask.php?id=" . $row['idtask'] . "\"" . "'>";
                            }
                            if ($row['status'] == "new") {
                                echo "<div class='hnotice hnotice-new' onclick='" . "location.href=\"cttask.php?id=" . $row['idtask'] . "\"" . "'>";
                            }
                            if ($row['status'] == "rejected") {
                                echo "<div class='hnotice hnotice-rejected' onclick='" . "location.href=\"cttask.php?id=" . $row['idtask'] . "\"" . "'>";
                            }
                            if ($row['status'] == "waiting") {
                                echo "<div class='hnotice hnotice-waiting' onclick='" . "location.href=\"cttask.php?id=" . $row['idtask'] . "\"" . "'>";
                            }
                            if ($row['status'] == "inprogress") {
                                echo "<div class='hnotice hnotice-inprogress' onclick='" . "location.href=\"cttask.php?id=" . $row['idtask'] . "\"" . "'>";
                            }
                            if ($row['status'] == "canceled") {
                                echo "<div class='hnotice hnotice-cancel' onclick='" . "location.href=\"cttask.php?id=" . $row['idtask'] . "\"" . "'>";
                            }
